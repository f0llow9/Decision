import { getDecisions } from "@/lib/data";
import { DecisionCard } from "@/components/cards/DecisionCard";
import { CreateBar } from "@/components/dashboard/CreateBar";
import { Layers } from "lucide-react";

export const dynamic = "force-dynamic";

export default async function DecisionsPage() {
  const decisions = await getDecisions();

  return (
    <div className="space-y-8 pb-20">
      <header>
        <div className="flex items-center gap-3 mb-1 text-blue-600">
          <Layers size={24} />
          <h1 className="text-xl md:text-2xl font-bold text-gray-900">消费决策</h1>
        </div>
        <p className="text-gray-400 text-xs md:text-sm truncate">
          管理你的每一个购买念头，理性评估，拒绝冲动消费。
        </p>
      </header>

      {/* Quick Create */}
      <section className="sticky top-0 z-20 pt-0 pb-4 -mx-4 px-4 md:-mx-8 md:px-8 bg-gray-50/95 backdrop-blur-md border-b border-gray-200/50 shadow-sm !mt-0">
        <CreateBar defaultType="decision" hideTypeSelector />
      </section>

      {/* List */}
      <section>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {decisions.map((decision) => (
            <div key={decision.id} className="h-full">
              <DecisionCard
                id={decision.id}
                name={decision.name}
                score={decision.score}
                status={decision.status as any}
                price={decision.price}
                heartbeatCount={decision.heartbeatCount}
                behaviorTotal={decision.behaviorTotalAmount}
                updatedAt={new Date(decision.updatedAt).toLocaleDateString()}
              />
            </div>
          ))}
          
          {decisions.length === 0 && (
            <div className="col-span-full py-20 text-center text-gray-400 bg-gray-50 rounded-3xl border border-dashed border-gray-200">
              <p>暂无决策记录，试着添加一个？</p>
            </div>
          )}
        </div>
      </section>
    </div>
  );
}
